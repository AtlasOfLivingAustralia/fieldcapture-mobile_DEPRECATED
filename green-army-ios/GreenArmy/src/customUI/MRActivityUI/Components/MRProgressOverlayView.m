        ...
        if ([keyPath isEqualToString:@"titleLabel.text"]) {
            if ([change[NSKeyValueChangeNotificationIsPriorKey] boolValue]) {
                ...
                if (!_savedAttributes) {
                    ...
                    #if DEBUG
                        DebugLog(@"** WARNING - Instance of %@ used automatically setTitleLabelText: internally, instead of titleLabel.text, but some text attributes may been lost.",
...
- (void)setStopBlock:(MRProgressOverlayViewStopBlock)stopBlock {
    ...
    if ([self.modeView conformsToProtocol:@protocol(MRStopableView)]
        ...
        #if DEBUG
            DebugLog(@"** WARNING - %@: %@ is only valid to call when the mode view supports %@ declared in %@!",
                  NSStringFromClass(self.class),
...
- (void)setProgress:(float)progress animated:(BOOL)animated {
    if ([self.modeView respondsToSelector:@selector(setProgress:animated:)]) {
        ...
        if (animated) {
            #if DEBUG
                DebugLog(@"** WARNING - %@: %@ is only valid to call when receiver is in a determinate mode or custom view supports %@!",
                      NSStringFromClass(self.class),
                      NSStringFromSelector(_cmd),
        ...
        #if DEBUG
            DebugLog(@"** ERROR - %@: %@ or %@ are only valid to call when receiver is in a determinate mode!",
                  NSStringFromClass(self.class),
